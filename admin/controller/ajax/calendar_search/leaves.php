...
foreach ($result as $row) {
    ...
    $sql = "SELECT a.id , a.company_name, a.appointment_date, b.f_first_name,b.f_last_name FROM sales_appointment a LEFT JOIN employees b ON a.employee_id = b.f_id";
    ...
    foreach ($result as $row) {
        $leave_arr[] = array(
            ...
            'title' => $row['f_first_name'] . ' '. $row['f_last_name'],
            'start' => date('Y-m-d',strtotime($row['appointment_date'])),
            'end' => date('Y-m-d',strtotime($row['appointment_date'])),
            'url' => "../controller/ajax/calendar_search/appointment.php?id=".$row['id']
